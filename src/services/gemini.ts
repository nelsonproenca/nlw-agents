...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const prompt: string = "Transcreva o áudio para o português do Brasil. " +
                         "Seja preciso e natural na transcrição. " +
                         "Mantenha a pontuação adequada e " +
                         "divida o texto em parágrafos quando for apropriado"
  const response = await gemini.models.generateContent({
    model, 
    contents: [ { text: prompt}, { inlineData: { mimeType, data: audioAsBase64 }}]
  })
